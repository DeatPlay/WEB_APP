/**
 * Self-contained Node.js icon generator script for Grammi Web App.
 * Can be run in environments with @resvg/resvg-js, sharp, or canvas.
 */
const fs = require('fs');
const path = require('path');

const SVG_TEMPLATE = (scale, offset) => `
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="512" height="512">
  <defs>
    <linearGradient id="brandGradient" x1="0%" y1="100%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#0d9488" />
      <stop offset="50%" stop-color="#14b8a6" />
      <stop offset="100%" stop-color="#2dd4bf" />
    </linearGradient>
  </defs>
  <rect width="512" height="512" fill="#0b0f17" />
  <g transform="translate(${offset}, ${offset}) scale(${scale})">
    <path fill="url(#brandGradient)" d="M504.3 11.1C493.3-1.6 474.5-3.7 461 6.2L252.3 160l144.9 0L502.6 54.6c11.8-11.8 12.6-30.8 1.6-43.5zM32 192c-17.7 0-32 14.3-32 32s14.3 32 32 32c0 82.5 43.4 147.7 123.9 176.2c-11.1 13.9-19.4 30.3-23.9 48.1C127.6 497.4 142.3 512 160 512l192 0c17.7 0 32.4-14.6 28.1-31.7c-4.5-17.8-12.8-34.1-23.9-48.1C436.6 403.7 480 338.5 480 256c17.7 0 32-14.3 32-32s-14.3-32-32-32L32 192z"/>
  </g>
</svg>
`;

console.log('Icon templates available in icons/icon.svg and icons/icon-maskable.svg.');
